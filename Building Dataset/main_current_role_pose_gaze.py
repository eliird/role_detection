...
from dataloader import PoseGazeCRData as customDataset
...
batch_size = 16
learning_rate = 1e-6
input_size = 12+360 + 3*60*66
output_size = 9
hidden_sizes = [4192,1024, 512, 256, 128, 64]  # Specify the structure of hidden layers
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (gaze, pose,cr, label) in enumerate(train_loader):
        ...
        pose = pose.to(device)
        cr = cr.to(device)
        #print(gaze.shape, pose.shape, cr.shape)
        label = label.to(device)

        if (gaze.shape[0]!= batch_size):
            continue

        output = model([gaze, pose, cr])
...
def val(testLoader, model):
    ...
    predictions = []
    labeledData = []
    for i, (gaze, pose,cr, label) in enumerate(testLoader):
    ...
    labeledData = labeledData.cpu().numpy()
    predictions = predictions.cpu().numpy()
    #print(labeledData, predictions)
    #mat = cf(labeledData, predictions)
    #report = cr(labeledData, predictions)
    #print(report)
    acc = correct / total
    #print(mat)
    mat = []
